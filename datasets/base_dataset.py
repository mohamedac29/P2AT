...
class BaseDataset(data.Dataset):
    ...
    def gen_sample(self, image, label,
        ...
        # Sobel operator for x and y
        sobelx = cv2.Sobel(label, cv2.CV_64F,1,0, ksize=5)
        sobely = cv2.Sobel(label, cv2.CV_64F,0,1, ksize=5)
        ...
        _,edge = cv2.threshold(edge,50,1,cv2.THRESH_BINARY)
