...
def train(config, epoch, num_epoch, epoch_iters, base_lr,
    ...
    for i_iter, batch in enumerate(trainloader, 0):
        for param in model.parameters():
            param.grad = None
        images, labels, b_gts, _, _ = batch
        images = images.cuda()
        labels = labels.long().cuda()
        b_gts = b_gts.float().cuda()

        with autocast():  # Enable mixed precision
            losses, _, acc, loss_list = model(images, labels, b_gts)
            loss = losses.mean()
            acc = acc.mean()

        scaler.scale(loss).backward()  # Scale the loss and call backward
        scaler.step(optimizer)  # Unscale the gradients and call optimizer.step
...
def validate(config, testloader, model, writer_dict):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(testloader):
            image, label, b_gts, _, _ = batch
            size = label.size()
            image = image.cuda()
            label = label.long().cuda()
            b_gts = b_gts.float().cuda()

            losses, pred, _, _ = model(image, label, b_gts)
